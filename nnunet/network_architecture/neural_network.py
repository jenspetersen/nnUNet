...
from deeputil.model.attention import ConvAttention2d
...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(
        ...
        assert step_size <= 1, (
            "step_size must be smaller than 1. Otherwise there will be a gap between consecutive "
            "predictions"
        )
        ...
        with context():
            with torch.no_grad():
                ...
                elif self.conv_op in (nn.Conv2d, ConvAttention2d):
                    if use_sliding_window:
    ...
    def predict_2D(
        ...
        assert step_size <= 1, (
            "step_size must be smaler than 1. Otherwise there will be a gap between consecutive "
            "predictions"
        )
        ...
        with context():
            with torch.no_grad():
                if self.conv_op == nn.Conv2d:
                    if use_sliding_window:
                        res = self._internal_predict_2D_2Dconv_tiled(
                            x,
                            step_size,
                            do_mirroring,
                            mirror_axes,
                        ...
                        res = self._internal_predict_2D_2Dconv(
                            x,
                            patch_size,
                            do_mirroring,
                            mirror_axes,
    ...
    def _internal_predict_3D_3Dconv_tiled(
        ...
        if use_gaussian and num_tiles > 1:
            ...
            gaussian_importance_map = torch.from_numpy(gaussian_importance_map).cuda(
    ...
    def _internal_predict_2D_2Dconv(
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_2D(
            data[None], mirror_axes, do_mirroring, None
        )[0]

        slicer = tuple(
            [
                slice(0, predicted_probabilities.shape[i])
                for i in range(len(predicted_probabilities.shape) - (len(slicer) - 1))
            ]
            + slicer[1:]
        )
    ...
    def _internal_maybe_mirror_and_pred_3D(
        ...
        result_torch = torch.zeros(
            [1, self.num_classes] + list(x.shape[2:]), dtype=torch.float
    ...
    def _internal_maybe_mirror_and_pred_2D(
        ...
        x = to_cuda(maybe_to_torch(x), gpu_id=self.get_device())
        result_torch = torch.zeros(
            [x.shape[0], self.num_classes] + list(x.shape[2:]), dtype=torch.float
        ).cuda(self.get_device(), non_blocking=True)
    ...
    def predict_3D_pseudo3D_2Dconv(
        self,
        x: np.ndarray,
        min_size: Tuple[int, int],
        do_mirroring: bool,
